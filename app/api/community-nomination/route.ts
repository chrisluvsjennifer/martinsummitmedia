...
export async function POST(request: Request) {
  try {
    ...
    const { yourName, yourEmail, nomineeName, nomineeCity, nomineeState, situation, whyShare } = body
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        reply_to: yourEmail,
        subject: `Community Story Nomination - ${nomineeName}`,
        html: `
          <!DOCTYPE html>
          <html>
            <head>
            ...
            <body>
              <div class="container">
                <div class="header">
                  <h1>COMMUNITY STORY NOMINATION</h1>
                </div>
                <div class="contact-info">
                  <h2>Nominee: ${nomineeName}</h2>
                  <p><strong>Location:</strong> ${nomineeCity}, ${nomineeState}</p>
                  <p><strong>Submitted by:</strong> ${yourName}</p>
                  <p><strong>Email:</strong> ${yourEmail}</p>
                </div>
                <div class="content">
                  <div class="field">
                    <div class="field-label">What They Are Going Through</div>
                    <div class="message-box">${situation.replace(/\n/g, "<br>")}</div>
                  </div>
                  
                  <div class="divider"></div>
                  
                  <div class="field">
                    <div class="field-label">Why Their Story Should Be Shared</div>
                    <div class="message-box">${whyShare.replace(/\n/g, "<br>")}</div>
                  </div>
                </div>
              </div>
            </body>
